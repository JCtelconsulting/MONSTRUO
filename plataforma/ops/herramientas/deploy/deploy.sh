...
if [ "$BRANCH" = "main" ]; then
  CANDIDATES+=(
    "$APP_DIR/plataforma/ops/env/.env.server"
    "$APP_DIR/.env.server"
    "$APP_DIR/.env"
  )
else
  CANDIDATES+=(
    "$APP_DIR/plataforma/ops/env/.env.server.dev"
...
if [ -n "$DEPLOY_SERVICE" ]; then
  echo "[deploy] rebuild selectivo: service=$DEPLOY_SERVICE"
  APP_GIT_SHA="$APP_GIT_SHA" APP_GIT_BRANCH="$APP_GIT_BRANCH" APP_BUILD_TIME="$APP_BUILD_TIME" ENV_FILE="$DEPLOY_ENV_FILE" STACK_NAME="$DEPLOY_STACK_NAME" "${COMPOSE_BIN[@]}" "${COMPOSE_ARGS[@]}" up -d --build "$DEPLOY_SERVICE"
else
...
ENV_FILE="$DEPLOY_ENV_FILE" STACK_NAME="$DEPLOY_STACK_NAME" "${COMPOSE_BIN[@]}" "${COMPOSE_ARGS[@]}" logs --tail=120 gateway || true
exit 1
