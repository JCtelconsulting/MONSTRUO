...
- 2026-04-01: DEV reordena módulos legado en apps canónicas: `pmo`, `ia`, `zabbix` y `fundacion` pasan a tener carpeta propia (`main.py + router.py + ui/`), `docker-compose` suma servicios dedicados para esos módulos, el sidebar local deja de apuntar a `/modulos/...` y la raíz activa deja fuera `static_ui`/`static` para reducir ambigüedad estructural durante el split.
- 2026-04-01: Canon de UI compartida en DEV: `gateway/shared/ui` queda oficializado como fuente única para assets compartidos de `gateway/ticketera/erp/bodega/crm`; las apps activas dejan de resolver `static_ui` y `code/static` para `shared`, y la migración restante de módulos legacy bajo `/modulos` queda explícitamente como deuda transicional separada.
- 2026-03-30: DEV microapps: `login/dashboard` de DEV se alinea al `gateway` como fachada compartida para `auth/session/ops`; el shell público del dashboard queda sincronizado y `ops/dashboard` usa fallback legado temporal mientras ERP/CRM no terminen de poblar la base nueva.
- 2026-03-30: Shell lateral unificado en DEV: `sidebar.js` pasa a ser el dueño único de `render + active + collapse + footer`, `admin.js` deja de tocar estado visual del sidebar y se normalizan `data-current-module`, assets compartidos y footer shell entre `dashboard/fundacion/ticketera/erp/bodega/crm` para evitar regresiones por copias y rutas divergentes.
- 2026-03-30: DEV microapps: se formaliza `gateway` como control plane compartido de `login/configuracion/ticketera` para autenticación, usuarios y `system_settings`, evitando desfases entre módulos separados durante la migración a apps independientes.
...
- Deploy por rama:
  - `main` -> `plataforma/ops/env/.env.server`
  - `dev` -> `plataforma/ops/env/.env.server.dev`
- Runtime y scripts operativos no deben hardcodear rutas como `/srv/monstruo_dev/.env.server.dev`; deben resolver por `ENV_FILE` o por la convención `plataforma/ops/env/`.
...
Nota: esta sección es histórica. El proxy actual ya no usa este modelo como referencia operativa principal.
...
Resumen corto:
- activos en proxy: `monstruo.conf`, `terreneitor.conf`, `sapa.conf`
- PROD usa `/`
- DEV usa `/dev/`
- Monstruo y Terreneitor ya quedaron unificados

## Política de Autenticación y Entornos — 2026-04-01

### Login central obligatorio
- Toda app web sin sesión válida debe redirigir al login central canónico:
  - PROD: `https://login.telconsulting.cl/`
  - DEV: `https://login.telconsulting.cl/dev/`
- Queda prohibido dejar logins locales por módulo como entrypoint público cuando existe SSO/cookie compartida.

### Regla de prefijo por entorno
- DEV usa prefijo público `/dev`.
- PROD no usa `/prod` ni ningún prefijo visible equivalente.
- Cualquier helper, redirect, asset compartido o navegación debe respetar esa convención exacta.

### Regla visual para producción
- Producción no debe mostrar badges, labels o botones que anuncien `DEV` por defecto.
- Los controles para volver de `DEV` a `PROD` pueden existir solo dentro del propio entorno `DEV`.
