...
*   `ui/`: Los archivos estáticos compartidos y las vistas transversales del sistema (HTML/CSS/JS).
...
*   **Login:** La página de inicio de sesión (`ui/login`).
*   **Dashboard de Operaciones:** El tablero principal que consolida KPIs (`ui/dashboard`).
*   **Configuración:** Panel administrativo para gestionar usuarios y roles (`ui/configuracion`).
*   **Shared (El Estándar Visual):** La carpeta `ui/shared/ui/` es la **fuente canónica de recursos compartidos** (estilos base `monstruo.css`, scripts comunes `sidebar.js`, `admin.js`, utilidades). Todas las demás aplicaciones (ej. `ticketera`) apuntan a esta carpeta compartida para cargar la shell común (sidebar + topbar) y mantener la coherencia visual.