...
CREATE TABLE IF NOT EXISTS fundacion.sesion_dia (
    ...
    notas                   TEXT,
    -- Trazabilidad
    creado_por              INTEGER REFERENCES fundacion.users(id) ON DELETE SET NULL,
    creado_at               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    actualizado_por         INTEGER REFERENCES fundacion.users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS fundacion.sesion_bloque_materiales (
    id                      SERIAL PRIMARY KEY,
    sesion_bloque_id        INTEGER NOT NULL REFERENCES fundacion.sesion_bloque(id) ON DELETE CASCADE,
    product_id              INTEGER,  -- separacion: sin FK a bodega (catalogo de Monstruo)
    nombre_libre            TEXT,
