...
# Uso: bash ops/scripts/qa/demo_flujo_completo.sh
set -uo pipefail
cd "$(dirname "$0")/../../.."
...
docker exec -i terreneitor-app-dev python - < ops/scripts/qa/seed_dev.py 2>&1 | grep -i "seed ok"

echo "### 2) Generar fotos demo (con/sin EXIF, formatos)"
docker exec -i terreneitor-app-dev python - <<'PYEOF' 2>&1 | grep -i "fotos demo"
import os
