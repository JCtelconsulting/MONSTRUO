...
with sync_playwright() as p:
    b = p.chromium.launch()
    ctx = b.new_context(
        ignore_https_errors=True, viewport={"width": 420, "height": 1000}
    )
    pg = ctx.new_page()
    errs = []
    ...
    pg.wait_for_timeout(1200)
    print("tarjetas grandes:", pg.locator(".guia-tarea-card").count())
    pg.screenshot(path=f"{OUT}/tarjetas_grandes.png")
