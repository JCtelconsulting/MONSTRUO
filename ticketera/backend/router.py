...
@router.post("/tickets/{ticket_id}/trash", response_model=dict)
async def trash_ticket(
    ticket_id: int,
    body: TicketTrashIn,
    sess: dict = Depends(deps.require_permission("tickets:write"))
):
    try:
        return tickets_service.move_ticket_to_trash(
            ticket_id=ticket_id,
            actor_id=sess["username"],
            actor_role=_session_actor_roles(sess),
...
@router.post("/tickets/{ticket_id}/restore", response_model=dict)
async def restore_ticket(
    ticket_id: int,
    sess: dict = Depends(deps.require_permission("tickets:write"))
):
    try:
        return tickets_service.restore_ticket_from_trash(
            ticket_id=ticket_id,
            actor_id=sess["username"],
            actor_role=_session_actor_roles(sess),
        )
...
@legacy_router.get("/tablero", response_model=dict)
@router.get("/tablero", response_model=dict)
async def get_tablero(
    sess: dict = Depends(deps.require_permission("tickets:read"))
):
    """Agrupación por estados para el Kanban.
    Los tickets 'cerrado' solo aparecen si fueron cerrados hace menos de 72 horas,
    evitando que se acumulen indefinidamente en el tablero.
    """
        ...
        if estado == "cerrado":
            ...
            if resolved_at:
                try:
                    ...
                    if dt < cutoff_72h:
                        continue  # Ya pasaron 72 horas, no aparece en Kanban
                except Exception:
                    pass  # Si no se puede parsear la fecha, lo mostramos igual
...
@router.get("/directorio/clientes", response_model=dict)
async def get_directorio_clientes(
    q: Optional[str] = Query(None, description="Buscar por nombre de cliente"),
    sess: dict = Depends(deps.require_permission("tickets:read"))
):
...
async def get_directorio_tickets(
    customer_id: Optional[str] = Query(None),
    estado: Optional[str] = Query(None),
    created_after: Optional[str] = Query(None, description="ISO date, ej: 2026-01-01"),
    created_before: Optional[str] = Query(None, description="ISO date, ej: 2026-03-31"),
    q: Optional[str] = Query(None),
...
@router.get("/directorio/metricas", response_model=dict)
async def get_directorio_metricas(
    customer_id: Optional[str] = Query(None),
...
@legacy_router.get("/stats", response_model=dict)
...
@legacy_router.get("/asignacion/timeline", response_model=dict)
...
@legacy_router.get("/dashboard/kpi", response_model=dict)
...
# ==========================================================================
@legacy_router.get("/notificaciones", response_model=dict)
...
# ==========================================================================
@legacy_router.get("/especialidades", response_model=dict)
...
@router.get("/ops/console", response_model=dict)
async def get_ops_console(
    limit: int = Query(50, ge=1, le=200),
    sess: dict = Depends(deps.require_permission("tickets:read"))
):
    """Obtiene datos para la consola de administración/estado."""
    return tickets_service.get_ops_console_data(limit=limit)
...
async def get_monthly_report(
    ...
    """Obtiene el informe estadístico mensual."""
    return tickets_service.get_monthly_report_data(year=year, month=month)
